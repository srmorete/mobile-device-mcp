...
import { filterUITree } from "../src/filter/filter";
import type { AndroidNode, IOSNode, RawUITree } from "../src/filter/types";
...
describe("iOS nodes", () => {
  test("button is normalized", () => {
    const result = filterUITree(tree([iosNode({ label: "Tap me", elementType: 8 })]));
    expect(result).toHaveLength(1);
  ...
  test("input prefers value over label", () => {
    const result = filterUITree(
      tree([iosNode({ label: "Username", value: "john@example.com", elementType: 48 })]),
    );
  ...
  test("non-input prefers label over value", () => {
    const result = filterUITree(
      tree([iosNode({ label: "Status", value: "Active", elementType: 47 })]),
    );
  ...
  test("link is clickable", () => {
    const result = filterUITree(tree([iosNode({ label: "Click here", elementType: 41 })]));
    expect(result[0].clickable).toBe(true);
    ...
    const result = filterUITree(
      tree([
        iosNode({
          label: "Container",
          elementType: 47,
          children: [iosNode({ label: "Child Button", elementType: 8 })],
        }),
      ]),
    );
...
describe("visibility", () => {
  ...
  test("center off screen bottom → not visible", () => {
    const r = filterUITree(
